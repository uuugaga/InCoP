...
class CamEncode_DINOv3(nn.Module):
    """DINOv3-small image encoder adapter for LSS-style BEV lifting.

    The DINO/ViT trunk produces patch tokens. We reshape patch tokens into a
    feature map, upsample them to the LSS feature resolution, and keep the
    original LSS image/depth heads. This preserves the downstream lift-splat
    interface: [B*N, C, D, fH, fW].
    """

    def __init__(self, D, C, downsample, ddiscr, mode,
                 use_gt_depth=False, depth_supervision=True, dino_args=None):
        super(CamEncode_DINOv3, self).__init__()
        try:
            import timm
        except ImportError as exc:
            raise ImportError(
                "CamEncode_DINOv3 requires timm. Install timm or use "
                "camera_encoder: EfficientNet/Resnet101."
            ) from exc
        ...
        self.model_name = dino_args.get("model_name", "vit_small_patch16_dinov3")
        ...
        self.num_prefix_tokens = int(getattr(self.trunk, "num_prefix_tokens", 1))

        adapter_channels = int(dino_args.get("adapter_channels", 512))
        self.adapter = nn.Sequential(
            nn.Conv2d(self.embed_dim, adapter_channels, kernel_size=1, bias=False),
            nn.BatchNorm2d(adapter_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(adapter_channels, adapter_channels, kernel_size=3,
    ...
    def get_dino_features(self, x):
        ...
        if H % self.patch_size[0] != 0 or W % self.patch_size[1] != 0:
            raise ValueError(
                f"DINOv3 input size {(H, W)} must be divisible by patch size "
                f"{self.patch_size}. Adjust final_dim or use another model."
            )
        ...
        if isinstance(tokens, dict):
            ...
            if tokens is None:
                raise RuntimeError(
                    "Unsupported DINOv3/timm forward_features dict output."
                )
        patch_tokens = tokens[:, self.num_prefix_tokens:, :]
        fH = H // self.patch_size[0]
        fW = W // self.patch_size[1]
        features = patch_tokens.transpose(1, 2).reshape(-1, self.embed_dim, fH, fW)

        target_h = H // self.downsample
        target_w = W // self.downsample
    ...
    def forward(self, x):
        x_img_ = x[:, :3, :, :]
        features = self.get_dino_features(x_img_)
        x_img = self.image_head(features)
...
class CamEncode_TimmFeatureMap(nn.Module):
    """Generic timm feature-map camera encoder for LSS-style BEV lifting.

    This is intended for CNN-like backbones such as
    convnext_tiny.dinov3_lvd1689m. The backbone returns dense feature maps
    directly, so no ViT token reshaping is needed.
    """

    def __init__(self, D, C, downsample, ddiscr, mode,
                 use_gt_depth=False, depth_supervision=True, timm_args=None):
        super(CamEncode_TimmFeatureMap, self).__init__()
        try:
            import timm
        except ImportError as exc:
            raise ImportError(
                "CamEncode_TimmFeatureMap requires timm. Install timm or use "
                "camera_encoder: EfficientNet/Resnet101."
            ) from exc
        ...
        out_indices = timm_args.get("out_indices", [1])
        if isinstance(out_indices, int):
            out_indices = [out_indices]
        out_indices = tuple(int(index) for index in out_indices)
        if len(out_indices) != 1:
            raise ValueError(
                "CamEncode_TimmFeatureMap expects exactly one out_indices value."
            )
        ...
        in_channels = int(self.trunk.feature_info.channels()[0])
        reduction = int(self.trunk.feature_info.reduction()[0])
        if reduction != self.downsample:
            raise ValueError(
                f"{self.model_name} out_indices={out_indices} has reduction "
                f"{reduction}, but LSS img_downsample is {self.downsample}."
            )

        adapter_channels = int(timm_args.get("adapter_channels", in_channels))
        self.adapter = nn.Sequential(
...
class BevEncodeSSFusion(nn.Module):
    ...
    def __init__(self, fusion_args):  # inC: 64  outC: not 1 for object detection
        ...
        if fusion_args['core_method'] == "v2xvit":
            self.fuse_module = V2XViTFusion(args['v2xvit'])
        else:
            raise ValueError(f"Unsupported fusion core_method: {fusion_args['core_method']}")
...
class BevEncodeMSFusion(nn.Module):
    ...
    def __init__(self, fusion_args):  # inC: 64  outC: not 1 for object detection
        ...
        raise ValueError(
            "BevEncodeMSFusion only supported the removed max_ms/att_ms "
            "fusion modules. Use a maintained fusion encoder."
        )
