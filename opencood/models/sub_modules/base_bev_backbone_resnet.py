...
class ResNetBEVBackbone(nn.Module):
    ...
    @staticmethod
    def align_upsample_outputs(ups):
        """Align multi-scale BEV decoder outputs before channel concat.

        Front-only IsaacSim BEV grids can be 224x224. Some downsample /
        transposed-conv branches differ by one cell for those shapes
        (for example 223 vs 224). Resize decoder branches to the largest
        branch shape so train and inference keep the configured BEV grid.
        """
        if len(ups) <= 1:
            return ups
        target_h = max(x.shape[-2] for x in ups)
        target_w = max(x.shape[-1] for x in ups)
        return [
    ...
    def forward(self, data_dict):
        ...
        ups = self.align_upsample_outputs(ups)
        if len(ups) > 1:
