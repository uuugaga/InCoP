...
class HeterModelLate(nn.Module):
    ...
    def _find_encoder(core_method):
        ...
        for module_name in ("opencood.models.heter_encoders",):
    ...
    def forward(self, data_dict):
        ...
        debug_ego_bev_feature = None
        if getattr(self, "save_feature_debug", False):
            debug_ego_bev_feature = feature[0].detach().float().cpu()
        ...
        if debug_ego_bev_feature is not None:
            output_dict["debug_features"] = {
                "ego_bev_feature": debug_ego_bev_feature,
                "ego_head_feature": feature[0].detach().float().cpu(),
            }
