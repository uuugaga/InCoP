...
def init_multiclass_result_stat(class_names, iou_thresholds=(0.25, 0.3, 0.4, 0.5, 0.7)):
    return {
        thresh: {
...
def calculate_tp_fp_multiclass_isaac(
    ...
    for class_id, class_name in enumerate(class_names):
        ...
        if iou_thresh not in result_stat:
            result_stat[iou_thresh] = {
                name: {"tp": [], "fp": [], "gt": 0, "score": []}
                for name in class_names
            }
        if class_name not in result_stat[iou_thresh]:
