...
def normalize_pairwise_tfm_range(pairwise_t_matrix, cav_lidar_range):
    """Normalize pairwise transforms for arbitrary BEV ranges.

    ``normalize_pairwise_tfm`` assumes the BEV canvas is centered at the
    local coordinate origin. IsaacSim uses a front-only canvas
    ``x=[0, xmax]``, so the feature-map center is not the robot origin. This
    function conjugates the physical transform with the BEV range center before
    converting it to the normalized coordinates expected by ``affine_grid``.
    """
    x_min, y_min, _, x_max, y_max, _ = cav_lidar_range
    width = float(x_max - x_min)
    height = float(y_max - y_min)
    center_x = float((x_max + x_min) / 2.0)
    center_y = float((y_max + y_min) / 2.0)

    affine_matrix = pairwise_t_matrix[:, :, :, [0, 1], :][:, :, :, :, [0, 1, 3]].clone()
    r00 = affine_matrix[..., 0, 0].clone()
    r01 = affine_matrix[..., 0, 1].clone()
    tx = affine_matrix[..., 0, 2].clone()
    r10 = affine_matrix[..., 1, 0].clone()
    r11 = affine_matrix[..., 1, 1].clone()
    ty = affine_matrix[..., 1, 2].clone()

    affine_matrix[..., 0, 0] = r00
    affine_matrix[..., 0, 1] = r01 * height / width
    affine_matrix[..., 1, 0] = r10 * width / height
    affine_matrix[..., 1, 1] = r11
    affine_matrix[..., 0, 2] = (r00 * center_x + r01 * center_y + tx - center_x) / (width / 2.0)
    affine_matrix[..., 1, 2] = (r10 * center_x + r11 * center_y + ty - center_y) / (height / 2.0)
