...
def coord_3d_to_2d(gt_box3d, int_matrix, ext_matrix, image_H=600, image_W=800, image=None, idx=None):
    ...
    if DEBUG:
        ...
        debug_dir = os.path.join("outputs", "camera_debug")
        ...
        plt.savefig(
            os.path.join(debug_dir, f"image_gt_box2d_{idx}.png"), dpi=300
        )
        plt.clf()
        plt.imshow(fg_mask)
        plt.savefig(
            os.path.join(debug_dir, f"image_gt_box2d_{idx}_mask.png"),
            dpi=300,
        )
        plt.clf()
