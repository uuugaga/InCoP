...
class CenterHeadLoss(nn.Module):
    def __init__(self, args):
        ...
        hm_loss_args = args.get("hm_loss", {}) or {}
        if isinstance(hm_loss_args, str):
            hm_loss_args = {"type": hm_loss_args}
        self.hm_loss_type = str(hm_loss_args.get("type", "focal")).lower()
        self.hm_kl_window_radius = int(hm_loss_args.get("window_radius", 4))
        self.hm_kl_pred_temperature = float(hm_loss_args.get("pred_temperature", 1.0))
        self.hm_kl_target_power = float(hm_loss_args.get("target_power", 1.0))
        self.hm_kl_bg_weight = float(hm_loss_args.get("background_weight", 0.05))
        self.hm_kl_bg_alpha = float(hm_loss_args.get("background_alpha", 2.0))
        self.hm_kl_bg_threshold = float(hm_loss_args.get("background_threshold", 1.0e-4))
        self.loss_dict = {}
    ...
    def _build_targets(self, pred_dict, target_dict):
        ...
        for b in range(batch_size):
            ...
            for box_i, box in enumerate(valid_boxes):
                ...
                target_classes[b, out_idx] = cls_id
        ...
        return heatmap, target_boxes, inds, masks, target_classes
    ...
    def _heatmap_loss(self, pred_logits, target, inds=None, masks=None, target_classes=None):
        if self.hm_loss_type in ("gaussian_kl", "kl", "distribution_kl"):
            return self._gaussian_kl_loss_center_net(
                pred_logits,
                target,
                inds,
                masks,
        ...
        return self._focal_loss_center_net(pred_logits, target)

    def _gaussian_kl_loss_center_net(self, pred_logits, target, inds, masks, target_classes):
        if inds is None or masks is None or target_classes is None:
            return self._focal_loss_center_net(pred_logits, target)
        batch_size, _, height, width = pred_logits.shape
        radius = max(int(self.hm_kl_window_radius), 1)
        temperature = max(float(self.hm_kl_pred_temperature), 1.0e-4)
        target_power = max(float(self.hm_kl_target_power), 1.0e-4)
        kl_losses = []

        for b in range(batch_size):
            valid_slots = torch.nonzero(masks[b], as_tuple=False).flatten()
            for slot in valid_slots:
                ind = int(inds[b, slot].detach().item())
                center_y = ind // width
                center_x = ind % width
                cls_id = int(target_classes[b, slot].detach().item())
                x0 = max(center_x - radius, 0)
                x1 = min(center_x + radius + 1, width)
                y0 = max(center_y - radius, 0)
                y1 = min(center_y + radius + 1, height)

                pred_patch = pred_logits[b, cls_id, y0:y1, x0:x1].reshape(-1)
                target_patch = target[b, cls_id, y0:y1, x0:x1].reshape(-1)
                target_prob = target_patch.clamp_min(1.0e-8).pow(target_power)
                target_prob = target_prob / target_prob.sum().clamp_min(1.0e-8)
                pred_log_prob = F.log_softmax(pred_patch / temperature, dim=0)
                kl_losses.append(
                    F.kl_div(pred_log_prob, target_prob, reduction="sum")
                )

        if kl_losses:
            object_loss = torch.stack(kl_losses).mean()
        else:
            object_loss = pred_logits.new_tensor(0.0)

        if self.hm_kl_bg_weight <= 0.0:
            return object_loss
        pred = torch.sigmoid(pred_logits).clamp(min=1.0e-4, max=1.0 - 1.0e-4)
        bg_mask = target <= self.hm_kl_bg_threshold
        if not bool(bg_mask.any().item()):
            return object_loss
        bg_loss = -torch.log(1.0 - pred) * torch.pow(pred, self.hm_kl_bg_alpha) * bg_mask
        bg_loss = bg_loss.sum() / bg_mask.float().sum().clamp_min(1.0)
        return object_loss + self.hm_kl_bg_weight * bg_loss

    def forward(self, output_dict, target_dict, suffix=""):
        prediction_key = f"center_head_preds{suffix}"
        pred_dict = output_dict.get(prediction_key, None)
        if pred_dict is None:
            if suffix:
                raise KeyError(
                    f"Missing {prediction_key} for CenterHead loss suffix {suffix!r}."
                )
            pred_dict = {
        ...
        heatmap, target_boxes, inds, masks, target_classes = self._build_targets(
            pred_dict, target_dict
        )
        hm_loss = self._heatmap_loss(
            pred_dict["hm"], heatmap, inds, masks, target_classes
        )
