...
class OPV2VBaseDataset(Dataset):
    def __init__(self, params, visualize, train=True):
        ...
        self.fixed_cav_order = bool(
            train_params.get('fixed_cav_order', False)
            or params.get('fixed_cav_order', False)
            or params.get('fusion', {}).get('dataset', '') == 'isaacsim'
        )
        fixed_ego_id = train_params.get(
            'fixed_ego_id', params.get('fixed_ego_id', None)
        )
        if self.fixed_cav_order and fixed_ego_id is None:
            fixed_ego_id = '0'
        self.fixed_ego_id = None if fixed_ego_id is None else str(fixed_ego_id)
        fixed_cav_ids_order = train_params.get(
            'fixed_cav_ids_order', params.get('fixed_cav_ids_order', [])
        )
        self.fixed_cav_ids_order = [str(cav_id) for cav_id in fixed_cav_ids_order]
        if self.fixed_ego_id is not None and self.fixed_ego_id not in self.fixed_cav_ids_order:
            self.fixed_cav_ids_order = [self.fixed_ego_id] + self.fixed_cav_ids_order
    ...
    def reinitialize(self):
        ...
        for (i, scenario_folder) in enumerate(self.scenario_folders):
            ...
            if self.train:
                ...
                cav_list = sorted(cav_list, key=self._cav_sort_key)
            if self.fixed_cav_order:
                cav_list = self._apply_fixed_cav_order(cav_list)
            ...
            be ego vehicle.
            """
            if not self.fixed_cav_order and int(cav_list[0]) < 0:
            ...
            for (j, cav_id) in enumerate(cav_list):
                ...
                print(f"scenario {i} cav {cav_id} yaml files before filtering: {len(yaml_files)}")
    ...
    @staticmethod
    def _cav_sort_key(cav_id):
        try:
            return (0, int(cav_id))
        except ValueError:
            return (1, str(cav_id))

    def _apply_fixed_cav_order(self, cav_list):
        cav_set = set(cav_list)
        ordered = [
            cav_id for cav_id in self.fixed_cav_ids_order
            if cav_id in cav_set
        ]
        ordered_set = set(ordered)
        remaining = [
            cav_id for cav_id in sorted(cav_list, key=self._cav_sort_key)
            if cav_id not in ordered_set
        ]
