...
class IsaacSimBaseDataset(OPV2VBaseDataset):
    ...
    def _patch_class_aware_label_collate(self):
        ...
        def generate_label_with_class_ids(*args, **kwargs):
            ...
            if self._isaac_last_class_ids is not None:
                ...
                anchor_class_labels = self._generate_anchor_class_labels(
                    label_dict,
                    kwargs.get("gt_box_center"),
                    kwargs.get("anchors"),
                    kwargs.get("mask"),
                )
                if anchor_class_labels is not None:
                    label_dict["anchor_class_labels"] = anchor_class_labels
            return label_dict
        ...
        def collate_batch_with_class_ids(label_batch_list):
            ...
            if label_batch_list and all(
                "anchor_class_labels" in item for item in label_batch_list
            ):
                label_torch_dict["anchor_class_labels"] = torch.from_numpy(
                    np.asarray(
                        [item["anchor_class_labels"] for item in label_batch_list],
                        dtype=np.int64,
                    )
                )
    ...
    def _generate_anchor_class_labels(self, label_dict, gt_box_center, anchors, mask):
        if gt_box_center is None or anchors is None or mask is None:
            return None
        if self._isaac_last_class_ids is None:
            return None

        pos_equal_one = label_dict.get("pos_equal_one")
        if pos_equal_one is None:
            return None

        anchor_class_labels = np.full(pos_equal_one.shape, -1, dtype=np.int64)
        gt_box_center = np.asarray(gt_box_center)
        mask = np.asarray(mask).astype(bool)
        class_ids = np.asarray(self._isaac_last_class_ids, dtype=np.int64)
        valid_gt = gt_box_center[mask == 1]
        valid_classes = class_ids[mask == 1]
        if valid_gt.shape[0] == 0:
            return anchor_class_labels

        feature_map_shape = anchors.shape[:2]
        anchor_num = anchors.shape[2]
        flat_anchors = anchors.reshape(-1, 7)
        gt_corners = box_utils.boxes_to_corners_3d(
            valid_gt, self.post_processor.params["order"]
        )
        anchor_corners = box_utils.boxes_to_corners_3d(
            flat_anchors, self.post_processor.params["order"]
        )
        gt_standup = box_utils.corner2d_to_standup_box(gt_corners)
        anchor_standup = box_utils.corner2d_to_standup_box(anchor_corners)
        iou = bbox_overlaps(
        ...
        id_pos, id_pos_gt = np.where(
            iou > self.post_processor.params["target_args"]["pos_threshold"]
        )
        id_pos = np.concatenate([id_pos, id_highest])
        id_pos_gt = np.concatenate([id_pos_gt, id_highest_gt])
        id_pos, unique_index = np.unique(id_pos, return_index=True)
        id_pos_gt = id_pos_gt[unique_index]
        if id_pos.shape[0] == 0:
            return anchor_class_labels
        ...
        anchor_class_labels[index_x, index_y, index_z] = valid_classes[id_pos_gt]
        return anchor_class_labels
    ...
    def get_last_valid_class_ids(self, object_ids):
        if self._isaac_last_class_ids is None:
            return np.zeros((len(object_ids),), dtype=np.int64)
        return np.asarray(self._isaac_last_class_ids[: len(object_ids)], dtype=np.int64)

    def set_last_class_ids_from_values(self, class_ids):
        padded = np.zeros(self.post_processor.params["max_num"], dtype=np.int64)
        class_ids = np.asarray(class_ids, dtype=np.int64).reshape(-1)
        count = min(class_ids.shape[0], padded.shape[0])
        if count > 0:
            padded[:count] = class_ids[:count]
        self._isaac_last_class_ids = padded
