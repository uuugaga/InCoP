...
from opencood.utils.transformation_utils import x1_to_x2, x_to_world, get_pairwise_transformation, get_pairwise_transformation_torch
from opencood.utils.pose_utils import add_noise_data_dict, generate_noise
from opencood.data_utils.pre_processor import build_preprocessor
...
def getIntermediateheterFusionDataset(cls):
    ...
    class IntermediateheterFusionDataset(cls):
        def __init__(self, params, visualize, train=True):
            ...
            pose_error_training_cfg = params.get("pose_error_training", {}) or {}
            self.teacher_noise_args = (
                pose_error_training_cfg.get("teacher_noise", {}) or {}
            )
            self.student_noise_args = (
                pose_error_training_cfg.get("student_noise", {}) or {}
            )
            pose_error_training_enabled = bool(
                pose_error_training_cfg.get(
                    "enabled",
                    bool(self.teacher_noise_args or self.student_noise_args)
                )
            )
            self.noisy_teacher_pose_training = bool(
                pose_error_training_cfg.get(
                    "noisy_teacher_pose_training",
                    pose_error_training_enabled
                    and bool(self.teacher_noise_args or self.student_noise_args)
                )
            )
            self.noisy_teacher_train_only = bool(
                pose_error_training_cfg.get("train_only", True)
            )
            self.student_extra_noise_args = (
                pose_error_training_cfg.get("student_extra_noise", {}) or {}
            )
            self.student_total_noise_args = (
                pose_error_training_cfg.get("student_total_noise", {}) or {}
            )
        ...
        @staticmethod
        def _noise_arg(noise_args, key, default=0.0):
            return float((noise_args or {}).get(key, default))

        def _resolve_teacher_noise_args(self):
            if self.teacher_noise_args:
                return self.teacher_noise_args
            return self.params.get('noise_setting', {}).get('args', {}) or {}

        def _resolve_student_total_noise_args(self):
            if self.student_noise_args:
                return self.student_noise_args
            return self.student_total_noise_args

        def _resolve_student_extra_noise_args(self):
            if self.student_extra_noise_args:
                return self.student_extra_noise_args

            total_args = self._resolve_student_total_noise_args()
            if not total_args:
                return self._resolve_teacher_noise_args()

            teacher_args = self._resolve_teacher_noise_args()
            total_pos_std = self._noise_arg(total_args, 'pos_std')
            total_rot_std = self._noise_arg(total_args, 'rot_std')
            teacher_pos_std = self._noise_arg(teacher_args, 'pos_std')
            teacher_rot_std = self._noise_arg(teacher_args, 'rot_std')
            return {
                'pos_std': math.sqrt(max(total_pos_std ** 2 - teacher_pos_std ** 2, 0.0)),
                'rot_std': math.sqrt(max(total_rot_std ** 2 - teacher_rot_std ** 2, 0.0)),
                'pos_mean': self._noise_arg(total_args, 'pos_mean') - self._noise_arg(teacher_args, 'pos_mean'),
                'rot_mean': self._noise_arg(total_args, 'rot_mean') - self._noise_arg(teacher_args, 'rot_mean'),
            }

        def _apply_noisy_teacher_pose_training(self, base_data_dict):
            if not self.noisy_teacher_pose_training:
                return
            if self.noisy_teacher_train_only and not self.train:
                return

            teacher_noise_args = self._resolve_teacher_noise_args()
            extra_noise_args = self._resolve_student_extra_noise_args()
            use_explicit_teacher_noise = bool(self.teacher_noise_args)

            for cav_content in base_data_dict.values():
                params = cav_content['params']
                if use_explicit_teacher_noise:
                    base_pose = np.asarray(
                        params.get('lidar_pose_clean', params['lidar_pose']),
                        dtype=np.float32
                    )
                    teacher_noise = generate_noise(
                        self._noise_arg(teacher_noise_args, 'pos_std'),
                        self._noise_arg(teacher_noise_args, 'rot_std'),
                        self._noise_arg(teacher_noise_args, 'pos_mean'),
                        self._noise_arg(teacher_noise_args, 'rot_mean'),
                    )
                    teacher_pose = base_pose + teacher_noise
                else:
                    teacher_pose = np.asarray(params['lidar_pose'], dtype=np.float32).copy()

                params['lidar_pose_teacher'] = teacher_pose.tolist()
                extra_noise = generate_noise(
                    self._noise_arg(extra_noise_args, 'pos_std'),
                    self._noise_arg(extra_noise_args, 'rot_std'),
                    self._noise_arg(extra_noise_args, 'pos_mean'),
                    self._noise_arg(extra_noise_args, 'rot_mean'),
                )
                params['lidar_pose'] = (teacher_pose + extra_noise).tolist()
        ...
        def get_item_single_car(self, selected_cav_base, ego_cav_base):
            ...
            ego_pose_teacher = ego_cav_base['params'].get(
                'lidar_pose_teacher', ego_pose_clean
            )
            selected_pose_teacher = selected_cav_base['params'].get(
                'lidar_pose_teacher',
            ...
            transformation_matrix_clean = \
                x1_to_x2(selected_pose_teacher,
                        ego_pose_teacher)
            ...
            if hasattr(self, "get_last_valid_class_ids"):
                single_class_ids = np.zeros(
                    self.params['postprocess']['max_num'], dtype=np.int64
                )
                valid_class_ids = self.get_last_valid_class_ids(object_ids)
                valid_class_ids = np.asarray(
                    valid_class_ids, dtype=np.int64
                ).reshape(-1)
                valid_count = min(
                    valid_class_ids.shape[0], single_class_ids.shape[0]
                )
                if valid_count > 0:
                    single_class_ids[:valid_count] = valid_class_ids[:valid_count]
                selected_cav_processed["single_object_class_ids"] = single_class_ids
            ...
            if hasattr(self, "get_last_valid_class_ids"):
                selected_cav_processed["object_class_ids"] = (
                    self.get_last_valid_class_ids(object_ids)
                )
        ...
        def __getitem__(self, idx):
            ...
            self._apply_noisy_teacher_pose_training(base_data_dict)
            ...
            object_stack = []
            object_id_stack = []
            object_class_id_stack = []
            single_label_list = []
            single_object_bbx_center_list = []
            single_object_bbx_mask_list = []
            single_object_class_id_list = []
            exclude_agent = []
            lidar_pose_list = []
            lidar_pose_clean_list = []
            lidar_pose_teacher_list = []
            cav_id_list = []
            ...
            for cav_id, selected_cav_base in base_data_dict.items():
                ...
                lidar_pose_teacher_list.append(
                    selected_cav_base['params'].get(
                        'lidar_pose_teacher',
                        selected_cav_base['params']['lidar_pose_clean'],
                    )
                )
            ...
            lidar_poses_teacher = np.array(lidar_pose_teacher_list).reshape(-1, 6)  # [N_cav, 6]
            ...
            for _i, cav_id in enumerate(cav_id_list):
                ...
                if not self.visualize:
                    label_sensor_type = "lidar" if sensor_type == "multimodal" else sensor_type
                    self.generate_object_center = eval(f"self.generate_object_center_{label_sensor_type}")
                ...
                if "object_class_ids" in selected_cav_processed:
                    object_class_id_stack += selected_cav_processed[
                        "object_class_ids"
                    ].tolist()
                ...
                if sensor_type == "lidar":
                    ...
                    eval(f"input_list_{modality_name}").append({
                        "lidar": selected_cav_processed[f"processed_features_{modality_name}"],
                        "camera": selected_cav_processed[f"image_inputs_{modality_name}"],
                    })
                else:
                    raise ValueError("Not support this type of sensor")
                ...
                if self.supervise_single or self.heterogeneous:
                    ...
                    if 'single_object_class_ids' in selected_cav_processed:
                        single_object_class_id_list.append(
                            selected_cav_processed['single_object_class_ids']
                        )
            ...
            if self.supervise_single or self.heterogeneous:
                ...
                if single_object_class_id_list:
                    single_view_update['single_object_class_ids_torch'] = torch.from_numpy(
                        np.array(single_object_class_id_list, dtype=np.int64)
                    )
                processed_data_dict['ego'].update(single_view_update)
            
            coop_object_class_ids = None
            ...
            if self.params['fusion']['dataset'] == 'dairv2x':
                ...
                # Exclude repeated objects while preserving deterministic
                # first-observation order across agents.
                seen_object_ids = set()
                unique_indices = []
                for object_idx, object_id in enumerate(object_id_stack):
                    if object_id in seen_object_ids:
                        continue
                    seen_object_ids.add(object_id)
                    unique_indices.append(object_idx)
                object_stack = np.vstack(object_stack)
                object_stack = object_stack[unique_indices]
                if object_class_id_stack:
                    object_class_id_stack = np.asarray(
                        object_class_id_stack, dtype=np.int64
                    )
                    coop_object_class_ids = object_class_id_stack[unique_indices]
                    if hasattr(self, "set_last_class_ids_from_values"):
                        self.set_last_class_ids_from_values(coop_object_class_ids)
            ...
            # 2D auxiliary supervision should be per image, but sourced from
            # the cooperative deduplicated 3D GT. Convert ego-frame boxes back
            # into each agent lidar frame before camera projection.
            aux_2d_object_bbx_center = np.zeros((cav_num, self.params['postprocess']['max_num'], 7))
            aux_2d_object_bbx_mask = np.tile(mask[None, :], (cav_num, 1))
            aux_2d_object_class_ids = np.zeros(
                (cav_num, self.params['postprocess']['max_num']), dtype=np.int64
            )
            valid_count = int(mask.sum())
            if coop_object_class_ids is not None and valid_count > 0:
                class_count = min(valid_count, coop_object_class_ids.shape[0])
                aux_2d_object_class_ids[:, :class_count] = coop_object_class_ids[:class_count][None, :]
            if valid_count > 0:
                order = self.params['postprocess']['order']
                coop_boxes_ego = object_bbx_center[:valid_count]
                coop_corners_ego = box_utils.boxes_to_corners_3d(coop_boxes_ego, order)
                ego_pose_clean = ego_cav_base['params']['lidar_pose_clean']
                for agent_idx, agent_pose_clean in enumerate(lidar_poses_clean):
                    if agent_idx == 0:
                        coop_boxes_agent = coop_boxes_ego
                    else:
                        ego_to_agent = x1_to_x2(ego_pose_clean, agent_pose_clean)
                        coop_corners_agent = box_utils.project_box3d(
                            coop_corners_ego, ego_to_agent
                        )
                        coop_boxes_agent = box_utils.corner_to_center(
                            coop_corners_agent, order
                        )
                    aux_2d_object_bbx_center[agent_idx, :valid_count, :] = coop_boxes_agent
            ...
            for modality_name in self.modality_name_list:
                if self.sensor_type_dict[modality_name] == "lidar":
                    ...
                    multimodal_inputs = eval(f"input_list_{modality_name}")
                    merged_lidar_inputs = merge_features_to_dict(
                        [item["lidar"] for item in multimodal_inputs]
                    )
                    merged_image_inputs_dict = merge_features_to_dict(
                        [item["camera"] for item in multimodal_inputs],
                        merge='stack',
                    )
                    processed_data_dict['ego'].update({
                        f'input_{modality_name}': {
                            'lidar': merged_lidar_inputs,
            ...
            processed_data_dict['ego'].update(
                ...
                'lidar_poses_teacher': lidar_poses_teacher,
                'lidar_poses': lidar_poses,
                'aux_2d_object_bbx_center_single': aux_2d_object_bbx_center,
                'aux_2d_object_bbx_mask_single': aux_2d_object_bbx_mask,
                'aux_2d_object_class_ids_single': aux_2d_object_class_ids})
        ...
        def collate_batch_train(self, batch):
            ...
            if self.supervise_single or self.heterogeneous:
                ...
                object_class_ids_single = []
                aux_2d_object_bbx_center_single = []
                aux_2d_object_bbx_mask_single = []
                aux_2d_object_class_ids_single = []
            ...
            for i in range(len(batch)):
                ...
                lidar_pose_teacher_list.append(
                    ego_dict.get('lidar_poses_teacher', ego_dict['lidar_poses_clean'])
                )
                ...
                if self.supervise_single or self.heterogeneous:
                    ...
                    if 'single_object_class_ids_torch' in ego_dict:
                        object_class_ids_single.append(
                            ego_dict['single_object_class_ids_torch']
                        )
                    if 'aux_2d_object_bbx_center_single' in ego_dict:
                        aux_2d_object_bbx_center_single.append(
                            torch.from_numpy(ego_dict['aux_2d_object_bbx_center_single'])
                        )
                        aux_2d_object_bbx_mask_single.append(
                            torch.from_numpy(ego_dict['aux_2d_object_bbx_mask_single'])
                        )
                        aux_2d_object_class_ids_single.append(
                            torch.from_numpy(ego_dict['aux_2d_object_class_ids_single'])
                        )
            ...
            for modality_name in self.modality_name_list:
                if len(eval(f"inputs_list_{modality_name}")) != 0:
                    if self.sensor_type_dict[modality_name] == "lidar":
                        ...
                        multimodal_inputs = eval(f"inputs_list_{modality_name}")
                        merged_lidar_inputs = merge_features_to_dict(
                            [item["lidar"] for item in multimodal_inputs]
                        )
            ...
            lidar_pose_teacher = torch.from_numpy(np.concatenate(lidar_pose_teacher_list, axis=0))
            label_torch_dict = \
            ...
            pairwise_t_matrix_clean = get_pairwise_transformation_torch(
                lidar_pose_teacher.float(), self.max_cav, record_len, 6
            ).to(dtype=pairwise_t_matrix.dtype)

            # add pairwise_t_matrix to label dict
            label_torch_dict['pairwise_t_matrix'] = pairwise_t_matrix
            label_torch_dict['pairwise_t_matrix_clean'] = pairwise_t_matrix_clean
            ...
            output_dict['ego'].update({'object_bbx_center': object_bbx_center,
                                    ...
                                    'pairwise_t_matrix': pairwise_t_matrix,
                                    'pairwise_t_matrix_clean': pairwise_t_matrix_clean,
                                    'lidar_pose_clean': lidar_pose_clean,
                                    'lidar_pose_teacher': lidar_pose_teacher,
            ...
            if self.supervise_single  or self.heterogeneous:
                single_output = {
                    'label_dict_single':{
                            ...
                            # for centerpoint
                            'object_bbx_center': torch.cat(object_bbx_center_single, dim=0),
                            'object_bbx_mask': torch.cat(object_bbx_mask_single, dim=0),
                ...
                if object_class_ids_single:
                    object_class_ids_single_torch = torch.cat(
                        object_class_ids_single, dim=0
                    )
                    single_output['label_dict_single'][
                        'object_class_ids_single'
                    ] = object_class_ids_single_torch
                    single_output['label_dict_single'][
                        'object_class_ids'
                    ] = object_class_ids_single_torch
                    single_output['object_class_ids_single'] = (
                        object_class_ids_single_torch
                    )
                if aux_2d_object_bbx_center_single:
                    single_output['aux_2d_object_bbx_center_single'] = torch.cat(
                        aux_2d_object_bbx_center_single, dim=0
                    )
                    single_output['aux_2d_object_bbx_mask_single'] = torch.cat(
                        aux_2d_object_bbx_mask_single, dim=0
                    )
                    single_output['aux_2d_object_class_ids_single'] = torch.cat(
                        aux_2d_object_class_ids_single, dim=0
                    )
                output_dict['ego'].update(single_output)
