...
class VoxelPostprocessor(BasePostprocessor):
    def __init__(self, anchor_params, train):
        ...
        default_r = self.params.get('dir_args', {}).get('anchor_yaw', [0])
        self.anchor_num = anchor_args.get('num', len(anchor_args.get('r', default_r)))
    ...
    def generate_anchor_box(self):
        ...
        w = self.params['anchor_args']['w']
        h = self.params['anchor_args']['h']
        r = self.params['anchor_args'].get(
            'r', self.params.get('dir_args', {}).get('anchor_yaw', [0])
        )
