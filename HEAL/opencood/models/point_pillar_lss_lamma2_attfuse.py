...
class PointPillarLSSLamma2AttFuse(nn.Module):
    """
    F-Cooper implementation with point pillar backbone.
    """
    def __init__(self, args):
        super(PointPillarLSSLamma2AttFuse, self).__init__()
        self.args = args
        ...
        for modality_name in self.modality_name_list:
            ...
            if model_setting.get("model_dir", False):
        ...
        if args['mm_fusion_method'] == "lamma3":
            self.mm_fusion3 = LAMMA3(args['lamma'], H=round(self.fH), W=round(self.fW))
        ...
        if self.mm_fusion_freeze:
            ...
            for param in self.mm_fusion3.parameters():
                param.requires_grad = False

        """
        multi-agent fusion
        """
        if args['ma_fusion_method'] == "max":
            self.ma_fusion = MaxFusion()
        if args['ma_fusion_method'] == "att":
            self.ma_fusion = AttFusion(args['att']['feat_dim'])
        if args['ma_fusion_method'] == "disconet":
            self.ma_fusion = DiscoFusion(args['disconet']['feat_dim'])
        if args['ma_fusion_method'] == "v2vnet":
            self.ma_fusion = V2VNetFusion(args['v2vnet'])
        if args['ma_fusion_method'] == 'v2xvit':
            self.ma_fusion = V2XViTFusion(args['v2xvit'])
        if args['ma_fusion_method'] == 'cobevt':
            self.ma_fusion = CoBEVT(args['cobevt'])
        if args['ma_fusion_method'] == 'where2comm':
            self.ma_fusion = Where2commFusion(args['where2comm'])
        if args['ma_fusion_method'] == 'who2com':
            self.ma_fusion = Who2comFusion(args['who2com'])
    ...
    def forward(self, data_dict):
        ...
        for modality_name in self.modality_name_list:   
            ...
            feature = eval(f"self.backbone_{modality_name}")({"spatial_features": feature})['spatial_features_2d']  # m1: torch.Size([4, 384, 128, 128]) m2: torch.Size([4, 384, 128, 128])
            
            if eval(f"self.shrinker_{modality_name}_freeze"):
                eval(f"self.shrinker_{modality_name}").eval()
            feature = eval(f"self.shrinker_{modality_name}")(feature)                                               # m1: torch.Size([4, 256, 64, 64]) m2: torch.Size([4, 256, 64, 64])
            feat_list["shrink_feat"+modality_name] = feature
            if eval(f"self.aligner_{modality_name}_freeze"):
                eval(f"self.aligner_{modality_name}").eval()
            feature = eval(f"self.aligner_{modality_name}")(feature)                                                # m1: torch.Size([4, 256, 64, 64]) m2: torch.Size([4, 256, 64, 64])
            feat_list["align_feat"+modality_name] = feature
        ...
        pc_feature = torch.stack(self.regroup(pc_feature, record_len)) # torch.Size([1, 4, 256, 64, 64]) no pooling applied
        img_fused_feature = torch.stack(self.regroup(img_fused_feature, record_len)) # torch.Size([1, 4, 256, 64, 64])
        mm_feature_2d, _, _ = self.mm_fusion3(pc_feature, img_fused_feature) # for verify modal features alignment
        # mm_feature_2d, _, _ = self.mm_fusion3(img_fused_feature, pc_feature) # torch.Size([4, 256, 64, 64])
        # test
        # mm_feature_2d, _, _ = self.mm_fusion(torch.zeros_like(img_fused_feature), pc_feature) # torch.Size([4, 256, 64, 64])
        ...
        if self.supervise_single:
            cls_preds_before_fusion = self.cls_head_single(mm_feature_2d)
            reg_preds_before_fusion = self.reg_head_single(mm_feature_2d)
            dir_preds_before_fusion = self.dir_head_single(mm_feature_2d)
        ...
        """
        fused_feature = self.ma_fusion(mm_feature_2d, record_len, affine_matrix) # torch.Size([1, 256, 64, 64])
