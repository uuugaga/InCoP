...
class YOLOFormatter:
    ...
    @staticmethod
    def _keep_classes():
        return list(getattr(config, "KEEP_CLASSES", []) or [])

    @staticmethod
    def _canonical_keep_class(class_name):
        ...
        for keep_class in keep_classes:
            if keep_class in names:
                return keep_class
        return None

    @staticmethod
    def _should_keep_class(class_name):
        return YOLOFormatter._canonical_keep_class(class_name) is not None
    ...
    def to_yolo_2d(bbox_data, img_w, img_h):
        ...
        for box in data:
            ...
            canonical_class_name = YOLOFormatter._canonical_keep_class(class_name)
            ...
            lines.append(f"{canonical_class_name} {class_id} {cx:.6f} {cy:.6f} {w:.6f} {h:.6f}")
        return lines
    ...
    def to_yolo_3d(bbox_3d_data):
        ...
        for box in data:
            ...
            line = f"{canonical_class_name} {class_id} {cx:.6f} {cy:.6f} {cz:.6f} {l:.6f} {w:.6f} {h:.6f} {yaw:.6f}"
            YOLOFormatter._debug_box(
                "3d",
                class_name,
                class_id,
...
class DataProcessor:
    ...
    def delete_frame(root, f_idx):
        ...
        file_paths = [
            ...
            f"data/lidar_full/{f_idx}.pcd",
            f"data/lidar_full/{f_idx}.npy",
    ...
    @staticmethod
    def save_frame(data, root, f_idx, pose_provider=None):
        ...
        lidar_full_data = data.get("lidar_full")
        if lidar_full_data is not None:
            DataProcessor.save_pcd(
                lidar_full_data,
                os.path.join(root, f"data/lidar_full/{f_idx}.pcd"),
            )
