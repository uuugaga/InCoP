...
class HeadlessVisualizer:
    ...
    def __init__(self, root_path, panels=None, show_text=True, bbox_pose_offset=0):
        ...
        self.bbox_pose_offset = int(bbox_pose_offset)
        ...
        map_dir = os.path.join(project_root, "map", "cropped_maps")
    ...
    def load_csv_record(self, csv_name, fid, offset=0):
    ...
    def load_pose_record(self, fid, offset=0):
        row = self.load_csv_record("pose.csv", fid, offset=offset)
    ...
    def render_frame(self, fid, temp_dir=None, output_path=None):
        ...
        def draw_3d_boxes(ax):
            ...
            bbox_pose_data = self.load_pose_record(fid, offset=self.bbox_pose_offset)
            if bbox_pose_data is None:
                bbox_pose_data = pose_data
            T_world_to_camera = self._pose_to_world_to_camera(bbox_pose_data, self.T_camera_to_base)
            edges = [
                [0, 1], [1, 2], [2, 3], [3, 0],
                [4, 5], [5, 6], [6, 7], [7, 4],
                [0, 4], [1, 5], [2, 6], [3, 7],
...
def make_video(image_folder, output_path, fps=10):
    """
    Build a video with FFmpeg glob mode so non-contiguous or nonzero-based
    frame numbering is handled correctly.
    """
    # Check that at least one input image exists.
...
def main(argv=None, force_show_text=None):
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Pose row offset used only for projecting 3D bounding boxes. Use 1 to test the next frame pose.",
    )
    ...
    for robot_root in robot_roots:
        vis = HeadlessVisualizer(
            robot_root,
            panels=panels,
            show_text=args.show_text,
            bbox_pose_offset=args.bbox_pose_offset,
        )
