...
DEFAULT_OBJECT_LIST = PROJECT_ROOT / "map" / "object_list.json"
DEFAULT_OUTPUT = DATASET_PARENT / "IsaacSimOPV2V"
...
# z up, matching the coordinate-frame notes in the project README.
...
def source_split_scenario_names(
    source: Path,
    split_key: str,
...
def load_depth_meters(src: Path) -> np.ndarray:
    ...
    # already encode meter values in the 0..255 range expected by this pipeline.
...
def parse_classes(raw: str) -> Optional[List[str]]:
    if raw.lower() in {"", "all", "*"}:
        return None
...
def default_keep_classes_arg() -> str:
    return ",".join(DEFAULT_KEEP_CLASSES)
...
def normalize_keep_classes(classes: Optional[Sequence[str]], asset_to_class: Dict[str, str]) -> Optional[List[str]]:
    if classes is None:
        return None
    normalized: List[str] = []
    seen = set()
    for class_name in classes:
        normalized_name = normalize_class_name(class_name, asset_to_class)
...
def filter_export_class_map(
    ...
    if not class_names:
        return {}, []
    if classes is None:
        filtered_class_names = list(class_names)
    else:
        available_class_names = set(class_names)
        filtered_class_names = [class_name for class_name in classes if class_name in available_class_names]
    if not filtered_class_names:
        raise SystemExit("No classes remain after applying --classes filter.")
    return {class_name: idx for idx, class_name in enumerate(filtered_class_names)}, filtered_class_names
...
def require_source_root(source: Optional[Path]) -> Path:
    if source is None:
        raise SystemExit(
            "--source is required; pass the generated Isaac Sim Dataset path."
        )
    resolved = source.expanduser().resolve()
    if not resolved.is_dir():
        raise SystemExit(f"Source Dataset directory not found: {resolved}")
    return normalize_source_root(resolved)
...
def prepare_output_roots(
    ...
    """Validate output roots and optionally replace their existing contents."""
    ...
    for path in paths:
        ...
        if requested.is_symlink():
            raise SystemExit(f"Refusing to replace symlinked output path: {requested}")
        resolved = requested.resolve()
        if resolved in resolved_paths:
            continue
        if (
            resolved == project_root
            or resolved == dataset_parent
            or resolved in project_root.parents
        ):
            raise SystemExit(f"Refusing to use protected output path: {resolved}")
        if resolved == source_root or resolved in source_root.parents:
            raise SystemExit(
                f"Refusing to use an output path that contains the source: {resolved}"
            )
    ...
    nonempty = [
        path for path in resolved_paths if path.exists() and any(path.iterdir())
    ]
    if nonempty and not overwrite:
        formatted = "\n  ".join(str(path) for path in nonempty)
        raise SystemExit(
            "Output directory is not empty. Choose a new output path or pass "
            f"--overwrite-output to replace it:\n  {formatted}"
        )
...
def argv_without_split(argv: Sequence[str]) -> List[str]:
    stripped: List[str] = []
    skip_next = False
    for arg in argv:
        if skip_next:
...
def required_split_scenario_names(
    source: Path,
    split_key: str,
    split_manifest: Optional[Path],
) -> Tuple[set[str], str]:
    if split_manifest is not None:
        manifest_path = split_manifest.expanduser().resolve()
        ...
        if not selected_names:
            raise SystemExit(
                f"No scenarios listed for split '{split_key}' in {manifest_path}"
            )
        return selected_names, f"split manifest {manifest_path}"
    ...
    if not selected_names:
        raise SystemExit(
            f"No scenarios listed for split '{split_key}' in source split JSON "
            f"files under {source}"
        )
    return selected_names, f"source split JSON groups={split_groups}"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Output root. Defaults to ../dataset/IsaacSimOPV2V.",
    )
    parser.add_argument("--split", choices=["train", "validate", "test", "all"], default="all", help="Dataset split to convert. Use all to convert train, validate, and test sequentially.")
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional JSON manifest with train/validate/test scenario lists. "
            "Without it, all source path_case_*_split.json files are required."
        ),
    )
    ...
    parser.add_argument("--classes", default=default_keep_classes_arg(), help="Comma-separated class names to keep from 3D labels, or 'all'. Defaults to DEFAULT_KEEP_CLASSES.")
    ...
    parser.add_argument(
        "--overwrite-output",
        action="store_true",
        help="Replace the exact non-empty OPV2V output roots before conversion.",
    )
    parser.add_argument(
        "--internal-split-run",
        action="store_true",
    ...
    source = require_source_root(args.source)
    output = args.output.expanduser().resolve()
    depth_output = depth_mirror_path(output).resolve()
    split_manifest = (
        args.split_manifest.expanduser().resolve()
    ...
    if args.split == "all":
        for split in ("train", "validate", "test"):
            required_split_scenario_names(source, split, split_manifest)
        prepare_output_roots(
            (output, depth_output),
            source,
            args.overwrite_output,
        )
        base_argv = argv_without_split(sys.argv[1:])
        ...
        for split in ("train", "validate", "test"):
            ...
            print(f"\n=== Converting split: {split} ===", flush=True)
    ...
    classes = normalize_keep_classes(classes, asset_to_class)
    class_to_id, class_names = filter_export_class_map(class_names, classes)
    ...
    selected_names, split_source = required_split_scenario_names(
        source,
        args.split,
        split_manifest,
    )
    ...
    print(
        f"Using {split_source} for split '{args.split}': "
        f"{len(selected_names)} scenario name(s)",
        flush=True,
    )
    ...
    if not args.internal_split_run:
        prepare_output_roots(
            (output, depth_output),
            source,
            args.overwrite_output,
