...
class CoverageSolver:
    def __init__(self, 
        ...
        os.makedirs(self.debug_dir, exist_ok=True)  # Shared debug directory
        ...
        # Relabel nodes sequentially as 0, 1, ..., n-1 while preserving
        # their original attributes, such as position.
    ...
    def plot_global_coverage(self):
        ...
        # Save under the shared debug directory.
    ...
    def debug_specific_edge(self, u, v):
        ...
        # Save under the debug directory and include u and v in the filename.
...
if __name__ == "__main__":
    ...
    # -----------------------------------------------------
    # 1. Render a map containing all edges and global node IDs.
    # At this point, all nodes are numbered from 0 to n-1.
    # -----------------------------------------------------
    solver.plot_global_coverage()
    
    # -----------------------------------------------------
    # 2. Optionally render debug images for several random edges.
    # -----------------------------------------------------
    solver.debug_precomputation(sample_size=5)
    
    # -----------------------------------------------------
    # 3. Render a dedicated debug image for a user-selected edge.
    # -----------------------------------------------------
    try:
        nodes_list = list(solver.G.nodes())
        if len(nodes_list) >= 2:
            # Nodes can now be selected directly by IDs such as 0, 1, 2, ...
            example_u = 16
            example_v = 41
