...
class JackalRobot:
    ...
    def get_linear_speed(self):
        velocity = np.asarray(self.robot_prim.get_linear_velocity())
    ...
    def get_data(self):
        pos, orient = self.get_current_pose()
    ...
    def prepare_record_data(self):
        ...
        data["lidar_full"] = current_pc.copy()
        ...
        if isinstance(data["lidar"], dict):
            cropped_lidar = dict(data["lidar"])
            cropped_lidar["data"] = pc_data
            data["lidar"] = cropped_lidar
        else:
            data["lidar"] = pc_data
    ...
    def save_prepared_data(self, save_root, data, timestamp_str=None, sim_time=None):
        ...
        DataProcessor.save_frame(data, save_root, timestamp_str, pose_provider=self.get_current_pose)
        return True
